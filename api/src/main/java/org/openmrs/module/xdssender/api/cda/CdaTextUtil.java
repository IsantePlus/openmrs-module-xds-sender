 ...
 */
@Component("xdsSender.CdaTextUtil")
public class CdaTextUtil {
	...
	public StructDocElementNode generateText(SubstanceAdministration administration, StructDocElementNode context) {
		...
		if (administration.getEffectiveTime().size() > 0)
			retVal.addElement("td").getChildren()
			        .addAll(this.generateIvlDisplay((SXCM<TS>) administration.getEffectiveTime().get(0)));
		else
		...
		// Drug
		if (administration.getConsumable() != null && administration.getConsumable().getManufacturedProduct() != null
		        && administration.getConsumable().getManufacturedProduct().getManufacturedDrugOrOtherMaterial() != null) {
			if (administration.getConsumable().getManufacturedProduct()
			        .getManufacturedDrugOrOtherMaterialIfManufacturedLabeledDrug() != null)
				retVal.addElement(
				    "td",
				    SD.createText("Labeled drug "),
				    SD.createText(administration.getConsumable().getManufacturedProduct()
				            .getManufacturedDrugOrOtherMaterialIfManufacturedLabeledDrug().getName().toString()));
			else
				retVal.getChildren().add(
				    this.createCodeTextCell(administration.getConsumable().getManufacturedProduct()
				            .getManufacturedDrugOrOtherMaterialIfManufacturedMaterial().getCode()));
		} else
		...
		if (administration.getEntryRelationship().size() > 0) {
			...
			StructDocElementNode subContext = this.createObservationTable(relationshipContent.addElement("list").addElement(
			    "item"));
	...
	private StructDocElementNode createSubstanceAdministrationTable(StructDocElementNode context) {
		...
		StructDocElementNode table = context.addElement("table"), thead = table.addElement("thead"), theadRow = thead
		        .addElement("tr"), tbody = table.addElement("tbody");
	...
	/**
	 * Create text for an organizer or template <list> <item> <caption>Organizer.code -
	 * Organizer.effectiveTime</caption> </item> </list>
	 */
			...
			if (organizer.getSubject().getRelatedSubject() != null) {
				...
				if (organizer.getSubject().getRelatedSubject().getSubject() != null) {
					if (organizer.getSubject().getRelatedSubject().getSubject().getName() != null)
						caption.addText(" "
						        + organizer.getSubject().getRelatedSubject().getSubject().getName().get(0).toString());
					if (organizer.getSubject().getRelatedSubject().getSubject().getAdministrativeGenderCode() != null)
						caption.addText(" "
						        + organizer.getSubject().getRelatedSubject().getSubject().getAdministrativeGenderCode()
						                .getCode());
					if (organizer.getSubject().getRelatedSubject().getSubject().getBirthTime() != null) {
						SimpleDateFormat fmt = new SimpleDateFormat(this.getDateFormat(organizer.getSubject()
						        .getRelatedSubject().getSubject().getBirthTime()));
						caption.addText(" DOB: "
						        + fmt.format(organizer.getSubject().getRelatedSubject().getSubject().getBirthTime()
						                .getDateValue().getTime()));
					}
				}
				if (organizer.getSubject().getRelatedSubject().getClassCode().getCode()
				        .equals(x_DocumentSubject.PersonalRelationship))
				...
				if (organizer.getSubject() != null && organizer.getSubject().getRelatedSubject() != null
					...
					caption.addText(String.format(" [%s] ", organizer.getSubject().getRelatedSubject().getCode()
					        .getDisplayName()));
			}
			caption.addText(")");
		}
	 ...
	 * <td>EXISTING TEXT AS COMMENT</td> </re>
	 */
	public StructDocElementNode generateText(Observation observation, StructDocElementNode context) {
		...
		if (observation.getValue() instanceof CV) {
			...
			if (reference != null)
				valueTd = retVal.addElement("td",
				    new StructDocElementNode("renderMultiMedia").addAttribute("referencedObject", reference.getValue()));
			else
				valueTd = retVal.addElement("td");
		...
		for (Participant2 ptcpt : observation.getParticipant()) {
			if (ptcpt.getParticipantRole() == null
			        || ptcpt.getParticipantRole().getPlayingEntityChoiceIfPlayingEntity() == null)
				continue;
			StructDocElementNode listItem = participantItems.addElement("item"), caption = listItem.addElement("caption");
			...
			// Actor info role
			if (ptcpt.getParticipantRole().getPlayingEntityChoiceIfPlayingEntity().getClassCode().getCode()
			        .equals(EntityClassRoot.ManufacturedMaterial))
				caption.addText("Manufactured Material");
			
			// Material code
			StructDocElementNode cell = this.createCodeTextCell(ptcpt.getParticipantRole()
			        .getPlayingEntityChoiceIfPlayingEntity().getCode());
	...
	private StructDocTextNode generateAuthorDisplay(Author authorInfo) {
		...
		if (authorInfo != null) {
			if (authorInfo.getAssignedAuthor().getAssignedAuthorChoiceIfAssignedPerson() != null)
				return new StructDocTextNode(authorInfo.getAssignedAuthor().getAssignedAuthorChoiceIfAssignedPerson()
				        .getName().get(0).toString());
			else
				return new StructDocTextNode(String.format("Device Authored : %s", authorInfo.getAssignedAuthor()
				        .getAssignedAuthorChoiceIfAssignedAuthoringDevice().getSoftwareName()));
		}
