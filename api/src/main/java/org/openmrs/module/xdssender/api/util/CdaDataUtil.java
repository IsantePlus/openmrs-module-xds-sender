...
public class CdaDataUtil {
    ...
    public AssignedEntity createAssignedEntity(Provider pvdr) {
        ...
        retVal.setId(SET.createSET(
                new II(config.getProviderRoot(), pvdr.getId().toString())
        ));
    ...
    public AssignedAuthor createAuthorPerson(Provider pvdr) {
        ...
        retVal.setId(SET.createSET(
                new II(config.getProviderRoot(), pvdr.getId().toString()),
                new II(config.getUserRoot(), Context.getUserService().getUsersByPerson(pvdr.getPerson(), false).get(0).getId().toString())
        ));
    ...
    public Organization createOrganization(Location location) {
        ...
        LocationAttribute externalId = metadataUtil.getLocationAttribute(location, XdsSenderConstants.ATTRIBUTE_NAME_EXTERNAL_ID);
        if (externalId != null)
        ...
        if (location.getParentLocation() != null) {
            ...
            externalId = metadataUtil.getLocationAttribute(location.getParentLocation(), XdsSenderConstants.ATTRIBUTE_NAME_EXTERNAL_ID);
            if (externalId != null)
                retVal.getAsOrganizationPartOf().getId().add(parseIIFromString(externalId.getValue().toString()));
            retVal.getAsOrganizationPartOf().getId().add(new II(config.getLocationRoot(), location.getParentLocation().getId().toString()));
    ...
    public Author getOpenSHRInstanceAuthor() {
        ...
        if (implementation == null) {
            ...
            // Set Id
            retVal.getAssignedAuthor().setId(SET.createSET(new II(config.getShrRoot(), implementation.getImplementationId())));
        }
    ...
    public CustodianOrganization getCustodianOrganization() {
        ...
        if (shrLocation == null) {
            ...
            LocationAttribute idAttribute = metadataUtil.getLocationAttribute(shrLocation, XdsSenderConstants.ATTRIBUTE_NAME_EXTERNAL_ID);
            if (idAttribute != null)
                retVal.setId(SET.createSET(
                        parseIIFromString(idAttribute.getValue().toString()),
    ...
    public Participant1 createRelatedPerson(Relationship relatedPerson, Patient recordTarget) {
        ...
        retVal.getAssociatedEntity().setCode(parseCodeFromString(relatedPerson.getRelationshipType().getDescription(), CE.class));
    ...
    public ANY getObservationValue(Obs obs) {
        ...
        if (obs.getValueBoolean() != null) {
            ...
            if (StringUtils.isBlank(numConcept.getUnits())) {
                ...
                return new PQ(BigDecimal.valueOf(obs.getValueNumeric()),
                        conceptUtil.getUcumUnitCode(numConcept));
